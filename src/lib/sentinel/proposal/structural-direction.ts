...
/** Minimum canonical ticks before structure is trusted. */
export const STRUCTURAL_MIN_TICKS = 300;
...
export function evaluateSideStructure(state: CanonicalStateLike, side: Side): SideStructure {
  ...
  // ── 2ND GREEN — parity, and never on the opposing parity (hard) ────────
  add("2ND GREEN", state.secondGreen, 12, (d) => {
    if (!favours(d)) {
      return {
        support: -1,
        blocking: true,
        note: `sits on the ${side} losing side — non-negotiable block.`,
      };
    }
    if (!spec.greenSet.includes(d)) {
  ...
  // ── RED — parity, excluded digit, never on the losing side (hard) ──────
  add("RED", state.red, 20, (d) => {
    if (d === spec.excludedRedDigit) {
      return {
        support: -1,
        blocking: true,
        note: `RED may never sit on digit ${spec.excludedRedDigit} for ${side}.`,
      };
    }
    if (!spec.redSet.includes(d)) {
      return {
        support: -1,
        blocking: true,
        note: `RED must sit on ${spec.redSet.join("/")} for ${side} — losing-side RED is a block.`,
      };
    }
    return { support: 1, note: `lowest-frequency digit correctly on the ${side} sacrificed set.` };
  });

  // ── 2ND RED — same rubric, lighter weight, still blocking ─────────────
  add("2ND RED", state.secondRed, 10, (d) => {
    if (d === spec.excludedRedDigit) {
      return {
        support: -1,
        blocking: true,
        note: `2ND RED may never sit on digit ${spec.excludedRedDigit} for ${side}.`,
      };
    }
    if (!spec.redSet.includes(d)) {
      return {
        support: -1,
        blocking: true,
        note: `2ND RED outside ${spec.redSet.join("/")} — losing-side block for ${side}.`,
      };
    }
    return { support: 1, note: `correctly placed for ${side}.` };
  });

  // ── MOST INCREASING — never on the losing side (hard) ─────────────────
  add("MOST INCREASING", state.mostIncreasing, 14, (d) => {
    if (!favours(d)) {
      return {
        support: -1,
        blocking: true,
        note: `strongest structural gain is on the ${side} losing side (${pp(delta(d))}) — block.`,
      };
    }
