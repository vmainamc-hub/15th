...
export function contractPsychology(
  ...
  // 2. RED and 2ND RED must strictly sit in range: 5-9 for OVER, 0-4 for UNDER.
  // 3. RED, 2ND RED, 2ND GREEN and MOST INCREASING may NEVER sit on the losing side.
  // 4. GREEN and RED must strictly respect side parity (OVER = GREEN even, RED odd; UNDER = GREEN odd, RED even).
  // 5. Structural Extreme Exhaustion Law:
  //    - For ALL OVER signals: Digit 0 MUST be >= 10.5% AND declining.
  //    - For ALL UNDER signals: Digit 9 MUST be >= 10.5% AND declining.
  // 6. GREEN on the losing side requires confirmed decay AND winning-side replacement.
  let hardBlock = false;
  let hardBlockReason: string | null = null;

  // 1. Excluded-digit check (1 for OVER, 8 for UNDER)
  if (state.red === excludedRedDigit) {
    hardBlock = true;
    hardBlockReason = `RED sits on the forbidden digit ${excludedRedDigit} for ${shape.side} psychology.`;
  }
  if (!hardBlock && state.secondRed === excludedRedDigit) {
    hardBlock = true;
    hardBlockReason = `2ND RED sits on the forbidden digit ${excludedRedDigit} for ${shape.side} psychology.`;
  }

  // 2. RED and 2ND RED range constraints (5-9 for OVER, 0-4 for UNDER)
  if (!hardBlock && state.red !== null) {
    if (isOver && (state.red < 5 || state.red > 9)) {
      hardBlock = true;
      hardBlockReason = `RED (digit ${state.red}) must sit between 5-9 for OVER contracts.`;
    } else if (!isOver && (state.red < 0 || state.red > 4)) {
      hardBlock = true;
      hardBlockReason = `RED (digit ${state.red}) must sit between 0-4 for UNDER contracts.`;
    }
  }
  if (!hardBlock && state.secondRed !== null) {
    if (isOver && (state.secondRed < 5 || state.secondRed > 9)) {
      hardBlock = true;
      hardBlockReason = `2ND RED (digit ${state.secondRed}) must sit between 5-9 for OVER contracts.`;
    } else if (!isOver && (state.secondRed < 0 || state.secondRed > 4)) {
      hardBlock = true;
      hardBlockReason = `2ND RED (digit ${state.secondRed}) must sit between 0-4 for UNDER contracts.`;
    }
  }

  // 3. Parity hard block checks for GREEN and RED
  if (!hardBlock && state.green !== null) {
    const greenParityOk = isOver ? isEven(state.green) : !isEven(state.green);
    if (!greenParityOk) {
      hardBlock = true;
      hardBlockReason = `GREEN (digit ${state.green}) has wrong parity for ${shape.side} psychology — must be ${isOver ? "EVEN (0/2/4/6/8)" : "ODD (1/3/5/7/9)"}.`;
    }
  }
  if (!hardBlock && state.red !== null) {
    ...
    if (!redParityOk) {
      hardBlock = true;
      hardBlockReason = `RED (digit ${state.red}) has wrong parity for ${shape.side} psychology — must be ${isOver ? "ODD (5/7/9)" : "EVEN (0/2/4)"}.`;
    }
  }

  // 3. Losing-side placement hard blocks
  const blockIfLosing = (digit: number | null, roleName: string) => {
    if (hardBlock || digit === null) return;
    const zone = zoneOfD(digit);
    if (zone === "LOSING") {
      hardBlock = true;
      hardBlockReason = `${roleName} (digit ${digit}) sits on the losing side — never permitted for ${shape.side} psychology.`;
    }
  };
  blockIfLosing(state.red, "RED");
  blockIfLosing(state.secondRed, "2ND RED");
  blockIfLosing(state.secondGreen, "2ND GREEN");
  blockIfLosing(state.mostIncreasing, "MOST INCREASING");
  ...
  // 4. Mandatory Extreme Exhaustion Law:
  // For ALL OVER: Digit 0 MUST be >= 10.5% AND declining.
  // For ALL UNDER: Digit 9 MUST be >= 10.5% AND declining.
  if (!hardBlock && state.n >= 300) {
    if (isOver) {
      const zeroShare = state.pct[0] ?? 0;
      const zeroDecaying = isGreenDecaying(0);
      if (zeroShare < EXTREME_SHARE_MIN_PCT || !zeroDecaying) {
        hardBlock = true;
        hardBlockReason = `OVER psychology structural law: Digit 0 must be >= ${EXTREME_SHARE_MIN_PCT}% and declining (currently ${zeroShare.toFixed(2)}%, ${zeroDecaying ? "declining" : "not declining"}).`;
      }
    } else {
      ...
      if (nineShare < EXTREME_SHARE_MIN_PCT || !nineDecaying) {
        hardBlock = true;
        hardBlockReason = `UNDER psychology structural law: Digit 9 must be >= ${EXTREME_SHARE_MIN_PCT}% and declining (currently ${nineShare.toFixed(2)}%, ${nineDecaying ? "declining" : "not declining"}).`;
      }
    }
  }

  // 5. GREEN on the losing side check
